...
public class DESlib {
    ...
    public static BitSet DES_decrypt(BitSet block, BitSet[] keys) {
        ...
        permutation(result, DESlib.IP); // Final permutation
    ...
    private static BitSet DESf(BitSet block, BitSet key){
        ...
        for(int i = 0; i < 48; i ++) {
            if(block.get(DESlib.E[i]-1) == true) {
                ep.set(i);
            }
        }
    ...
    private static BitSet SBoxSubstituion(BitSet input) {
        ...
        for(int i = 0; i < 8; i ++) {
            byte[] sbox = DESlib.S[i]; //S-Box for i-th segment
